...
app = FastAPI(
    ...
    description="""
# 🎓 AI-Powered Marksheet Data Extraction API

Professional-grade API for extracting structured data from academic marksheets using advanced AI models.
...
- **Smart Processing**: Automatic quality detection and routing
...
### Processing Methods
Our API intelligently selects the optimal processing method:

| Method | Use Case | Speed | Accuracy | Cost |
|--------|----------|-------|----------|------|
| `text_pdf_direct` | Text-based PDFs | ⚡ ~0.5s | ⭐⭐⭐⭐⭐ | 💰 Very Low |
| `ocr_text_llm` | High-quality images | ⚡⚡ ~5-8s | ⭐⭐⭐⭐ | 💰💰 Low |
| `vision_direct` | Low-quality images | ⚡⚡⚡ ~15-25s | ⭐⭐⭐⭐⭐ | 💰💰💰 Medium |
| `vision_ocr_fallback` | OCR failure cases | ⚡⚡⚡ ~15-25s | ⭐⭐⭐⭐⭐ | 💰💰💰 Medium |

### Confidence Scores
Every extracted field includes a confidence score (0-1) indicating reliability.
...
### Student Information
- Full Name, Roll Number, Registration Number
- Class/Grade, Section, Academic Year
- Date of Birth, Father's Name, Mother's Name

### Academic Performance
- Subject-wise marks (obtained, maximum, grade)
...
### Get Started
1. **Register**: Create an account at `/api/v1/auth/register`
2. **Login**: Get your JWT token at `/api/v1/auth/login`
3. **Use Token**: Include `Authorization: Bearer <token>` in headers

### Benefits of Authentication
- ✅ MongoDB logging of all extractions
...
### 1. Simple Extraction (No Auth)
```bash
curl -X POST "http://localhost:8000/api/v1/extract" \\
  -F "file=@marksheet.pdf"
```

### 2. Authenticated Extraction
```bash
# Get token
...
curl -X POST "http://localhost:8000/api/v1/extract/batch" \\
  -H "Authorization: Bearer YOUR_TOKEN" \\
  -F "files=@marksheet1.pdf" \\
  -F "files=@marksheet2.pdf" \\
  -F "files=@marksheet3.pdf"
```

## 🏗️ Architecture

Built with professional MVC architecture:
- **Middleware Layer**: JWT authentication, request tracking
- **Controller Layer**: Clean route handlers
- **Service Layer**: Business logic (extraction, logging, user management)
...
For issues or questions, check the `/api/health` endpoint for service status.
